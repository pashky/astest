...
File.open('sql-airports.dat', 'w') do |f|
  CSV.foreach(File.dirname(__FILE__) + "/airports.csv") do |ap|
    ...
    unless code.nil?
      name = ap[1..2].reverse.select {|n| !n.empty?}.join(" - ")
      latitude = ap[6]
      longitude = ap[7]
      f.puts "#{airport_id},#{code},#{name},#{latitude},#{longitude}\n"
      airport_id += 1
    end
  end
end
  ...
  airport_id.times do |origin_id|
    ...
    destination_ids.each do |destination_id|
      randr(10,100).times do
        ...
        value = randr(10,1000)
        f.puts "#{flight_id},#{origin_id},#{destination_id},#{departs_at.to_s},#{returns_at.to_s},#{value},#{weeks}\n"
        flight_id += 1
      end
    end
  end
